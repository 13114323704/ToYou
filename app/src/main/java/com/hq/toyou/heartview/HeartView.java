...
public class HeartView extends SurfaceView implements SurfaceHolder.Callback {
	...
	private void init() {
		...
		backgroundPaint.setColor(getResources().getColor(R.color.heartview_bg));
	...
	public Point getHeartPoint(float angle) {
		...
		float y = (float) (-heartRadio * (13 * Math.cos(t) - 5
				* Math.cos(2 * t) - 2 * Math.cos(3 * t) - Math.cos(4 * t)));
	...
	private Bloom getBloom(float angle) {
		...
		for (int i = 0; i < blooms.size(); i++) {
			...
			Point bp = b.getPoint();
			float distance = (float) Math.sqrt(Math.pow(p.x - bp.x, 2)
					+ Math.pow(p.y - bp.y, 2));
