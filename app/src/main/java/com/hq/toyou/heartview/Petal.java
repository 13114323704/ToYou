...
public class Petal {
	...
	// 构造函数，由花朵类调用
	public Petal(float stretchA, float stretchB, float startAngle, float angle,
				 int color, float growFactor) {
	...
	private void draw(Point p, Canvas canvas) {
		if (!isFinished) {
			...
			Point t = new Point(0, this.radius).rotate(MyUtil
					.degrad(this.startAngle));
